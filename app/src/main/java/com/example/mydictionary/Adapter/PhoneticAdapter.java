...
import com.example.mydictionary.ViewHolder.PhoneticViewHolder;

import java.util.List;

public class PhoneticAdapter extends RecyclerView.Adapter<PhoneticViewHolder> {
    private Context context;
    private List<Phonetics> list;

    public PhoneticAdapter(Context context, List<Phonetics> list) {
    ...
    public void onBindViewHolder(@NonNull PhoneticViewHolder holder, @SuppressLint("RecyclerView") int position) {
        holder.getTxt_phonetic().setText(list.get(position).getText());
        holder.getBtnaudio().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                MediaPlayer player = new MediaPlayer();
                try{
                    player.setAudioStreamType(AudioManager.STREAM_MUSIC);
                    player.setDataSource(list.get(position).getAudio());
                    player.prepare();
                    player.start();
                }
                catch (Exception e){
